...
### Issueに対するソリューションプロジェクトの管理について

※下記にさまざまなルール的なものが記載されておりますが、古橋が起こっているドキュメント管理の一例としてご覧ください。他に効率の良い管理方法があれば、下記のルールを無視していただいても構いません。  
基本的な考え方は、「作成した資料に、他の方がアクセスしやすい。また、時間が経った後でも、該当の資料が探しやすい管理方法」を行っていただければ結構です。
...
ソリューションてプロジェクとのディレクトリ名は、極力変更しないでください。

他のプロジェクトから別のプロジェクト内のドキュメントの参照をされることもあるかと思いますので、リンク切れを防ぐためです。
...
### READMEの作成, imagesディレクトリの作成

Issue管理用のディレクトリらを作成したら、そのディレクトリをどう使うか、の方針を記したREADMEを作成してください。

また、Markdown形式で画像を埋め込むことができますので、必要であればimagesディレクトリを作成し、画像を格納してください。できるだけドキュメントと同レベルのディレクトリには画像は格納せず、画像は画像で管理するようにした方が良いかと思います。格納する画像ファイル等も、参照元となるドキュメントと関連付けてナンバリングしておくと、後々の管理が楽になるかと思います。
